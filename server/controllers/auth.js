...
export const agents = async (req, res) => {
  try {
    const agents = await User.find({ role: "Official" }).select(
      "-password -role -followedReports -photo.key -photo.Key -photo.Bucket"
    );
    res.json(agents);
  } catch (err) {
    console.log(err);
...
export const agentAdCount = async (req, res) => {
  try {
    const reports = await Report.find({ reportedBy: req.params._id }).select("_id");
    res.json(reports);
  } catch (err) {
    console.log(err);
  }
...
export const agent = async (req, res) => {
  try {
    ...
    const reports = await Report.find({ reportedBy: user._id }).select(
      "-photos.key -photos.Key -photos.ETag -photos.Bucket -location"
    );
